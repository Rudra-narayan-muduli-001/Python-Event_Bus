...
class TelemetrySettings:
    ...
    # FG1/FG2/FG3 latency SLO targets (milliseconds)
    wake_detection_target_ms: int = 300        # FG1: wake word → ready
    speaker_verification_target_ms: int = 500  # FG1: voice biometric check
    vad_detection_target_ms: int = 100         # FG1: VAD decision latency
    stt_final_target_ms: int = 1500            # FG1: full speech-to-text
    llm_first_token_target_ms: int = 800        # FG2: time-to-first-token
    llm_total_target_ms: int = 6000           # FG2: total LLM inference
    tts_first_audio_target_ms: int = 400       # FG3: time-to-first-audio-chunk
    interrupt_response_target_ms: int = 200    # FG3: barge-in response latency
