...
class LoggerFactory:
    ...
    def register(self, name: str, logger: Logger) -> None:
        """Register a pre-built logger into the factory cache.

        Allows external builders (e.g. feature-group log formatters) to
        insert loggers that were constructed from Phase 0 primitives but
        assembled with FG-specific policies. Subsequent ``get(name)``
